...
def find_first_best_checkpoint(run_directory):
    ...
    if len(chkps) == 0:
        raise FileNotFoundError("The path " + checkpoint_path + "either does not exist or has no checkpoints")
...
if __name__ == '__main__':
    ...
    run_directory = f'{package_location}/real_runs/{run_name}'

    _, best_ckp = find_first_best_checkpoint(run_directory)
    ...
    for path_def in path_defintions:
        ...
        out_name = f'{name}-{datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")}'
        summary = SummaryWriter(f'{package_location}/runs/{out_name}')
